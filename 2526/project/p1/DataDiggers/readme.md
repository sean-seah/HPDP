# Project 1 - Optimizing High-Performance Data Processing for Large-Scale Web Crawlers

## Group Name: DataDiggers

WEBSITE: [BERNAMA](https://www.bernama.com/en/)

Group Members:

1. NURUL IKA SYAFINY BINTI AZHAR (A23CS0164)
2. LUBNA AL HAANI BINTI RADZUAN (A23CS0107)
3. NUR FIRZANA BINTI BADRUS HISHAM (A23CS0156)
4. NURAISYAH BINTI MOHD ZIKRE (A23CS0160)

## ✅ [Student Project Checklist & Timeline Tracker]

Use this checklist to ensure your group stays on track throughout the 4-week project.

### 📅 **Timeline & Checklist**

| Week | Task | Responsible Member(s) | Status |
|------|------|------------------------|:-----------:|
| Week 1 | Form a diverse group (4 members) | All | ✅ 23/5 |
| Week 1 | Choose a Malaysian website & get approval | All | ✅ 29/5 |
| Week 1 | Identify target data fields (≥100,000 records) | All | ✅ 29/5 |
| Week 1 | Design system architecture (crawler + pipeline) | Ika & Hani | ✅ 2/6 |
| Week 2 | Develop and test web crawler (initial batch) | Ika & Hani | ✅ 2/6 |
| Week 2 | Begin collecting real data (progressive storage) | Ika & Hani | ✅ 8/6 |
| Week 3 | Process and clean dataset (remove duplicates, standardize) | Firzana | ✅ 12/6 |
| Week 3 | Apply optimization (threading, Spark, Dask, etc.) | Nuraisyah | ✅ 12/6 |
| Week 3 | Benchmark performance (before vs after) | Firzana & Nuraisyah | ✅ 12/6 |
| Week 4 | Compile results, charts, and graphs | Documentation Lead | ☐ |
| Week 4 | Write final report | All (shared) | ☐ |
| Week 4 | Submit report to Turnitin (by 16 May) | Group Leader | ☐ |
| Week 4 | Submit code, dataset, and slides | All | ☐ |
| Week 4 | Present final project (10 minutes) | All | ☐ |
